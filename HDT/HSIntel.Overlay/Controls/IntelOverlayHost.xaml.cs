...
namespace HSIntel.Overlay.Controls
{
    public partial class IntelOverlayHost : UserControl
    {
        ...
        public void SetCoordinateMapper(IOverlayCoordinateMapper mapper)
        {
            ...
            // Force an initial visual build on first mapper assignment
            _lastSignature = null;
            try { System.Diagnostics.Trace.WriteLine("[HSIntel][Overlay] firstDrawAttempt"); } catch { }
            TryUpdateRecommendationVisuals();
        }
        ...
        private void OnSizeChanged(object sender, SizeChangedEventArgs args)
        {
            ...
            // DPI/size changes require a full refresh of retained visuals
            _lastSignature = null;
            TryUpdateRecommendationVisuals();
        }
        ...
        private void OnUnloaded(object sender, RoutedEventArgs e)
        {
            ...
            try { System.Windows.Media.CompositionTarget.Rendering -= OnCompositionTargetRendering; } catch { }
        }
        ...
        private void SetDebugMode(bool enabled)
        {
            ...
            if(enabled)
            {
                ...
                try { System.Diagnostics.Trace.WriteLine("[HSIntel][Overlay] DebugOverlayMode=ON"); } catch { }
            }
            else
            {
                try { System.Diagnostics.Trace.WriteLine("[HSIntel][Overlay] DebugOverlayMode=OFF"); } catch { }
            }
        }
        ...
        private void OnCompositionTargetRendering(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var visuals = Orders.Children.Count + Arrows.Children.Count + Halo.Children.Count + DebugLayer.Children.Count + 1;
                System.Diagnostics.Trace.WriteLine($"[HSIntel][Overlay] RenderTick size={size} dpi={dpi.DpiScaleX:0.##}x{dpi.DpiScaleY:0.##} payload{{badges={badges},arrows={arrows},halos={halos}}} visuals={visuals}");
            }
            catch { }
        }
        ...
        private void TryUpdateRecommendationVisuals()
        {
            try
            {
                ...
                if(actions == null || actions.Count == 0)
                {
                    ...
                    // Clear next action text to default
                    try { CoachHud.SetNextActionText(null); } catch { }
                ...
                // Update 'Next Action' with a succinct instruction for the first step
                try
                {
                    var first = actions[0];
                    var nextText = DescribeAction(first);
                    CoachHud.SetNextActionText(nextText);
                }
                catch { }
                ...
                try
                {
                    System.Diagnostics.Trace.WriteLine($"[HSIntel][Overlay] RecommendationChanged steps={actions.Count}");
                    System.Diagnostics.Trace.WriteLine($"[HSIntel][Overlay] Sig: changed=1 len={sig.Length} source=counts+positions+nonce");
                }
                catch { }
                RedrawFrom(actions);
            }
        ...
        private static string DescribeAction(GameAction a)
        {
            try
            {
                switch(a.ActionType)
                {
                    ...
                    {
                        var card = a.Card;
                        var name = SafeName(card?.CardName, card?.CardId, "card");
                    ...
                    {
                        var atk = a.Attacker;
                        var attackerName = SafeName(atk?.CardName, atk?.CardId, "minion");
        ...
        private static string DescribeTarget(ActionTarget t)
        {
            ...
            if(t.IsHero)
                return side + " hero";
            var name = SafeName(t.CardName, t.CardId, "minion");
            return side + " " + name;
        }
            ...
            for(int i = 0; i < actions.Count; i++)
            {
                ...
                switch(a.ActionType)
                {
                    case GameActionType.PlayCard:
                    {
                        ...
                        {
                            // ZonePosition is 1-based in HDT entities; convert to 0-based for our slot cache
                            var slot0 = Math.Max(0, Math.Min(9, Math.Max(1, slot1) - 1));
                        ...
                        try { System.Diagnostics.Trace.WriteLine($"[HSIntel][Overlay] Step#{order}: PlayCard id={a.Card?.EntityId?.ToString() ?? "_"} using={(haveSlot ? "ExactHandSlot" : (a.Card?.ZonePosition is int ? "ZonePosFallback" : "DefaultHandCenter"))}"); } catch { }
                        break;
                    }
                    case GameActionType.Attack:
                    {
        ...
        private Point ToLocal(IOverlayCoordinateMapper mapper, Point screenPoint)
        {
            // Prefer WPF's screen->element transform to account for per-monitor DPI, window offsets, and any future transforms.
            try
            {
            ...
            {
                // Fallback: simple offset from overlay window bounds
                var overlay = mapper.OverlayBounds;
