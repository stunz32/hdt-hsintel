...
namespace HSIntel.Engine.Evaluation
{
    ...
    public sealed class StateEvaluator
    {
        ...
        public EvaluationResult Evaluate(GameContext context, SimulationMetrics? metrics = null)
        {
            ...
            // Optional opponent-reads adjustment (ordering-only penalty) guarded by toggle.
            if(EngineToggles.EnableOpponentReads)
            {
                totalScore += ComputeOpponentReadsAdjustment(context);
            }
        ...
        private static double ComputeOpponentReadsAdjustment(GameContext context)
        {
            double adj = 0.0;
            try
            {
                // Simple, conservative signals:
                var oppHand = context.OpponentHand.TotalCardCount ?? context.OpponentHand.Cards.Count;
                var friendlyBoard = context.Board.FriendlyMinions.Count;
                var secretsInPlay = 0;
                foreach(var s in context.Board.Secrets)
                    if(s.Controller == HSIntel.Core.Models.Events.ParticipantSide.Opponent)
                        secretsInPlay++;

                // Penalize overextension into large opponent hand.
                if(friendlyBoard >= 4 && oppHand >= 6)
                    adj -= 0.15;

                // Small penalty when multiple opponent secrets are present.
                if(secretsInPlay >= 2)
                    adj -= 0.10;

                // Soft caution for being at low health vs. unknown hand.
                var friendlyHp = (context.Board.FriendlyHero.Health ?? 0) + (context.Board.FriendlyHero.Armor ?? 0);
                if(friendlyHp <= 10 && oppHand >= 5)
                    adj -= 0.05;
            }
            catch { }
            return adj;
