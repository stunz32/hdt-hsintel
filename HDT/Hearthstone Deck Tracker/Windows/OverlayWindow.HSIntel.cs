...
namespace Hearthstone_Deck_Tracker.Windows
{
    public partial class OverlayWindow
    {
        ...
        private void InitializeIntelCoachOverlay()
        {
            try
            {
                ...
                if(_intelOverlayHost == null)
                {
                    ...
                    Hearthstone_Deck_Tracker.Utility.Logging.Log.Info($"[HSIntel][Overlay] Host attached. mountZ={Panel.GetZIndex(IntelOverlayMount)} hostZ={Panel.GetZIndex(_intelOverlayHost)} size={IntelOverlayMount.ActualWidth:0}x{IntelOverlayMount.ActualHeight:0}");
                }

                SyncIntelOverlayHostLayout();

                // Ensure DebugOverlayMode is on for diagnostics
        ...
        private void SyncIntelOverlayHostLayout()
        {
            ...
            try
            {
                Hearthstone_Deck_Tracker.Utility.Logging.Log.Debug($"[HSIntel][Overlay] Host layout synced: host={width:0}x{height:0} mount={IntelOverlayMount.ActualWidth:0}x{IntelOverlayMount.ActualHeight:0} canvas={CanvasInfo.ActualWidth:0}x{CanvasInfo.ActualHeight:0}");
            }
            catch { }

            // Ensure layout materializes promptly so ActualWidth/Height are non-zero for early RenderTick
            try { _intelOverlayHost.UpdateLayout(); } catch { }
        }
