...
namespace Hearthstone_Deck_Tracker.HSIntel
{
	internal sealed class HdtGameStateSource : IGameStateSource
	{
		...
		private static HeroDescriptor? BuildHeroDescriptor(Player? player, ParticipantSide side)
		{
			...
			var heroCardName = heroEntity.LocalizedName ?? heroCardId;
			var heroClass = player?.CurrentClass ?? player?.OriginalClass;
			var (heroPowerCardId, heroPowerCardName) = FindHeroPowerDescriptor(player!);
		...
		private static (string? CardId, string? CardName) FindHeroPowerDescriptor(Player player)
		{
			var heroPowers = player.PlayerEntities
				.Where(e => e.IsHeroPower)
				.OrderByDescending(e => e.IsInPlay ? 3 : e.IsInSetAside ? 2 : e.IsInHand ? 1 : 0)
				.ThenByDescending(e => e.Id)
				.ToList();

			if(heroPowers.Count == 0)
				return (null, null);

			var heroPower = heroPowers.First();
			var cardId = heroPower.CardId ?? heroPower.Info.LatestCardId;
			var cardName = heroPower.LocalizedName ?? cardId;
