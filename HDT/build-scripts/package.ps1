...
if (!$dev) {
    & $signtool sign /tr "http://timestamp.digicert.com" /td SHA256 /a /f $cert /p $Env:CERT_PASSWORD "$hdtReleaseDir\HDTUninstaller.exe" "$hdtReleaseDir\Hearthstone Deck Tracker.exe" | Out-Default
