        ...
        private void LoadExplanations()
        {
            try
            {
                ...
                if(ExplainHeader != null)
                {
                    ...
                    var rngpProp = setType.GetProperty("RngProbability", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance); var rngp = (double)(rngpProp?.GetValue(set, null) ?? 0.0); ExplainHeader.Text = string.Format("Explanations (Conf {0:P0}{1}{2})", conf, lethal ? ", Lethal" : "", rngp > 0 ? string.Format(", RNG {0:P0}", rngp) : "");
                }
            }
            catch { }
        }
