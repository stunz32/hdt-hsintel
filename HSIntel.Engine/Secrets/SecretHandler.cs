...
{
    /// <summary>
    /// Stub-only secret summarizer and probe recommender.
    /// Produces logs for information-first sequencing; does not alter scoring.
    /// </summary>
    internal static class SecretHandler
    {
        public static void SummarizeOpponentSecrets(GameContext context)
        {
            ...
            var cards = opponentSecrets
                .Select(s => string.IsNullOrWhiteSpace(s.CardName) ? (s.CardId ?? "Secret") : s.CardName!)
                .ToList();
        ...
        /// <summary>
        /// Emits a conservative probe order focused on information-first, low-risk triggers.
        /// Does not change behavior; logs only.
        /// </summary>
        public static void EmitProbeOrder(GameContext context)
        {
            ...
            // Very coarse, class-agnostic probe sequence. Purposefully generic with no HearthDb dependency.
            var probes = new List<string>
            {
                ...
                "5) Proceed with core line if no triggers"
            };

            Trace.WriteLine($"[HSIntel][Engine] Secrets: probe-order start ({count} secrets)");
            foreach(var p in probes)
                Trace.WriteLine($"[HSIntel][Engine] Secrets: {p}");
